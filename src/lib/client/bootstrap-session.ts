 ...
 * cookie for a device-token bundle. RxDB replication then uses the Bearer
 * access token (no cookies required) — this is what makes sync work from
 * Capacitor WebViews and across origins.
 */
export async function bootstrapDeviceSession(args: {
  ...
  const bundle: StoredTokenBundle = { ...data, userId: args.userId };
