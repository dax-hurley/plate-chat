...
import { useMealPlanMutations } from "@/lib/stores";
...
function macroContributionForSlot(
  slot: MealPlanBoardViewJson["slots"][number],
  planEditMode: boolean,
  draftLibraryBySlotId: Record<string, string | null>
): MealLibraryItemJson | null {
  if (planEditMode) {
    const draft = assignmentFromDraft(slot, draftLibraryBySlotId);
    return draft === slot.libraryItemId && slot.libraryItem
      ? slot.libraryItem
      : null;
  }
...
function sumDayMacros(
  daySlots: MealPlanBoardViewJson["slots"],
  planEditMode: boolean,
  draftLibraryBySlotId: Record<string, string | null>
) {
  let calories = 0;
...
export function MealPlanBoard({
  ...
  /** When false, hides prev/next week links (e.g. onboarding single-week review). */
  showWeekNav?: boolean;
  /** When false, hides the "This week" title row in the plan card (e.g. onboarding). */
  showThisWeekHeader?: boolean;
}) {
  const { addMealPlanSlot, removeMealPlanSlot, saveMealPlanLibraryAssignments } =
          ...
          <p className="text-foreground flex min-h-[3.25rem] min-w-0 max-w-[46%] flex-1 flex-col items-center justify-center gap-0.5 px-1 text-center sm:max-w-none">
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold sm:text-sm">
              ...
              <span className="truncate leading-tight">
                {weekRangeLabel(weekStartDayKey)}
              </span>
            </span>
                ...
                >
                  <div className="relative">
                    ...
                    >
                      ...
                      >
                        <div className="flex min-w-0 min-h-0 flex-1 items-start gap-3">
                          <MealPlanDayIcon className="text-foreground mt-0.5 size-5 shrink-0 self-start sm:size-[1.35rem]" />
                        ...
                        {daySlots.length > 0 ? (
                          ...
                          >
                            <p className="text-muted-foreground mb-0.5 text-[0.65rem] font-medium sm:text-xs">
                              Day total
                            </p>
                            <MacroStrip
