...
import {
  actionFillSetSlot,
  actionLogDistanceTimeFromStopwatch,
  actionLogTimedSet,
...
export function TimeSetCountdown({
  ...
  const logDuration = useCallback(
    (seconds: number) => {
      runAction(() =>
        actionLogTimedSet(sessionId, exerciseId, setIndex, seconds)
      );
    },
    [exerciseId, runAction, sessionId, setIndex]
  );
...
export function DistanceStopwatch({
  ...
  function onLogSet() {
    ...
    if (logTimeInsteadOfDistance) {
      ...
      runAction(() => actionFillSetSlot(sessionId, exerciseId, setIndex));
    }
  }
