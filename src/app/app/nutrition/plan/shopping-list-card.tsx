...
import {
  actionGenerateShoppingList,
  actionRegenerateShoppingList,
      ...
      <CardContent className="space-y-4">
        {shoppingList.awaitingAiGeneration ? (
          <div className="space-y-3">
            <Button
              ...
              onClick={() => {
                void (async () => {
                  ...
                  try {
                    const result = await actionGenerateShoppingList(planId);
                    if (!result.ok) {
                    ...
                    toast.error("Couldn’t generate the shopping list.");
                  } finally {
            ...
            >
              <Sparkles
                className={`size-4 ${generatePending ? "animate-pulse" : ""}`}
                aria-hidden
              />
              {generatePending ? "Generating…" : "Generate shopping list"}
            </Button>
          ...
          <>
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              <Button
                ...
                disabled={regenPending || lineCount === 0}
                title={
                  lineCount === 0
                    ? "Assign meals first"
                ...
                onClick={() => {
                  void (async () => {
                    ...
                    try {
                      const result = await actionRegenerateShoppingList(planId);
                      if (!result.ok) {
                        toast.error(result.error);
                        return;
                      }
                      ...
                      toast.error("Couldn’t refresh the shopping list.");
                    } finally {
              ...
              >
                <RefreshCw
                  className={`size-4 ${regenPending ? "animate-spin" : ""}`}
                  aria-hidden
                />
                Regenerate list
              </Button>
